...
import { div, form, h4, input } from 'cycle-snabbdom';
import { Observable as $ } from "rx"
import {
  ABOUT_YOU, BIRTHDAY, CONTINUE, PERSONAL_DETAILS, PREFERRED_NAME, STEP_ABOUT, STEP_APPLIED,
  STEP_QUESTION, STEP_REVIEW, STEP_TEAM_DETAIL, STEP_TEAMS,
  USER_APPLICATION_ABOUT_SCREEN_BIRTHDAY_ERROR_SELECTOR,
  USER_APPLICATION_ABOUT_SCREEN_LEGAL_NAME_ERROR_SELECTOR,
  USER_APPLICATION_ABOUT_SCREEN_PHONE_ERROR_SELECTOR,
  USER_APPLICATION_ABOUT_SCREEN_PREFERRED_NAME_ERROR_SELECTOR,
  USER_APPLICATION_ABOUT_SCREEN_SUPERPOWER_ERROR_SELECTOR,
  USER_APPLICATION_ABOUT_SCREEN_ZIPCODE_ERROR_SELECTOR, USER_APPLICATION_ABOUT_CONTINUE_BUTTON_SELECTOR,
...
function renderAboutScreen(model) {
  const {
    opportunity: {
...
function render(model) {
  ...
  switch (step) {
    case STEP_ABOUT :
      return renderAboutScreen(model);
    case STEP_QUESTION:
    case STEP_TEAMS:
    ...
    default :
      ...
      throw 'render > internal error : unknown steps or step is not ABOUT as expected!!'
  }
}
...
export const processApplicationRenderAboutScreen = curry(_renderComponent);
